...
class RecommendationQualityTester:
    ...
    def load_test_data(self):
        ...
        for credentials in test_users:
            ...
            if login_response.status_code == 200:
                ...
                # Get user history
                history_response = requests.get(f"{API_URL}/api/users/{user_id}/history")
                if history_response.status_code == 200:
                    ...
                    print(f"✓ Loaded user: {username} ({len(liked_products)} likes, {len(viewed_products)} views)")
        
        print(f"\nTotal users loaded: {len(self.users)}")
    
    def calculate_precision_recall(self, recommended: Set[str], relevant: Set[str]) -> Tuple[float, float, float]:
        """
    ...
    def test_collaborative_filtering(self):
        ...
        for user in self.users:
            ...
            # Get recommendations
            rec_response = requests.get(f"{API_URL}/api/recommendations/{user_id}?n=10&method=collaborative")
            
            if rec_response.status_code == 200:
                recommendations = rec_response.json()["recommendations"]
                recommended_ids = set(rec["_id"] for rec in recommendations)
                
                # Use liked products as ground truth for relevant items
                relevant_ids = user["liked_products"]
                
                # Calculate metrics
                precision, recall, f1 = self.calculate_precision_recall(recommended_ids, relevant_ids)
                ...
                print(f"  Relevant (liked): {len(relevant_ids)} products")
    ...
    def test_content_based(self):
        ...
        for user in self.users:
            ...
            rec_response = requests.get(f"{API_URL}/api/recommendations/{user_id}?n=10&method=content")
            
            if rec_response.status_code == 200:
                recommendations = rec_response.json()["recommendations"]
                recommended_ids = set(rec["_id"] for rec in recommendations)
                
                # Use liked products as ground truth
                relevant_ids = user["liked_products"]
    ...
    def test_diversity(self):
        ...
        for user in self.users:
            ...
            if rec_response.status_code == 200:
                ...
                diversity_score = len(categories) / len(recommendations) if recommendations else 0
        ...
        if diversity_results:
            avg_diversity = statistics.mean(r["diversity_score"] for r in diversity_results)
    ...
    def test_coverage(self):
        ...
        coverage = len(all_recommended) / len(self.all_products) if self.all_products else 0
    ...
    def generate_report(self):
        ...
        if "collaborative_filtering" in self.results:
            ...
            print(f"Collaborative Filtering F1-Score: {Color.GREEN}{cf_f1:.3f}{Color.END}")
        ...
        if "collaborative_filtering" in self.results:
            cf_f1 = self.results["collaborative_filtering"]["average_f1_score"]
            if cf_f1 < 0.3:
                print(f"  {Color.YELLOW}• Low F1-Score: Consider using matrix factorization or deep learning{Color.END}")
            elif cf_f1 < 0.5:
                print(f"  {Color.YELLOW}• Moderate F1-Score: Fine-tune similarity thresholds{Color.END}")
            else:
                print(f"  {Color.GREEN}• Good F1-Score: System performing well{Color.END}")
        
        if "diversity" in self.results:
            diversity = self.results["diversity"]["average_diversity"]
            if diversity < 0.4:
                print(f"  {Color.YELLOW}• Low diversity: Add diversity penalty in ranking{Color.END}")
            else:
                print(f"  {Color.GREEN}• Good diversity: Recommendations cover multiple categories{Color.END}")
        
        if "coverage" in self.results:
            coverage = self.results["coverage"]["coverage_percentage"]
            if coverage < 30:
                print(f"  {Color.YELLOW}• Low coverage: Popular items dominate, consider popularity penalty{Color.END}")
            else:
                print(f"  {Color.GREEN}• Good coverage: Wide range of products recommended{Color.END}")
